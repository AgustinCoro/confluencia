import 'package:conciertos2/conciertos_database.dart';
...
Future <void> mostrarNotificacion(mensaje) async{
  ...
  await flutterLocalNotificationsPlugin.show(1, 'Aviso', mensaje, notificationDetails);
  ...
  for (Map<String, dynamic> row in result) {
    ...
    if (fechaDateTime.isBefore(fechaLDateTime) ||
        ...
        await db.update(
          ...
          where: 'id = ?', // Condición WHERE para seleccionar el registro específico
...
void obtenerRegistrosConDiferenciaDeTiempo() async {
  ...
  for (Map<String, dynamic> row in result) {
    ...
    if (fechaDateTime.isBefore(fechaLDateTime) ||
        ...
        mostrarNotificacion("En menos de 15 minitos estara " + row['banda'] + " en el escenario " + row['estadio']);
